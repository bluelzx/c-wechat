...
@Controller
@RequestMapping("flowActivityTest")
public class TestFlowActivityController extends BaseController {
	
	@Autowired
	private TestIFlowActivityService flowActvityService;
	...
	*/
	...
	public String index(@PathVariable String activityId, HttpServletRequest request, HttpServletResponse response,
	...
	*/
	...
	public Object openGroup(@PathVariable String activityId,
		...
		try {
			...
			resultMap.put("code", -1);
			resultMap.put("msg", e.getMessage() != null ? e.getMessage() : "系统异常");
			e.printStackTrace();
		}
		return resultMap;
	}
	...
	public void checkActivity(WechatUser userInfo, FlowActivity flowActivity,
		...
		try {
			if (null != list && list.size() > 1) {
				...
				List<FlowMembers> listMembers = flowMembersService
				...
				for (FlowMembers flowMembers : list) {
					if (flowMembers.getIsLeader().equals(0)) {
						if (flowMembers.getGroupId().equals(groupId)) {// 再次打开自己的参加过的页面							
							model.addAttribute("openUser", WebchatUtil
									.getUserInfo(flowGroups.getOpenid()));
			...
			// 参与活动			
			FlowMembers flowMembers = flowActvityService.joinGroup(flowGroups,flowActivity, userInfo);
	...
	*/
	...
	public String record(@PathVariable String groupId,HttpServletRequest request, HttpServletResponse response,
		...
		MemcachedUtil mem = MemcachedUtil.getInstance();
		FlowActivity flowActivity = (FlowActivity) mem.get(flowMembers
				.getActivityId());
		if (null == flowActivity) {
			flowActivity = flowActvityService.queryFLowActivityById(flowMembers
					.getActivityId());
